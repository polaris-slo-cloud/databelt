...
use skylark_manage::{delete_single_state, get_single_state, init_new_chain, skylark_manage_version, start_timing, store_single_state, SkylarkPolicy};
use std::env;
        ...
        (&Method::GET, "/get-and-set") => {
            ...
            let new_key = match store_single_state(state, &dest_node, &policy).await {
                Ok(key) => {
                    ...
                    debug!("store_state: skylark manage result: {:?}", key);
                    key
                }
                Err(e) => {
                    error!(
                        "store_state: Error calling skylark manage store state: {:?}",
                        e
                    );
                    return Ok(Response::builder()
            ...
            match delete_single_state(new_key).await {
                ...
                Err(e) => {
                    error!(
                        "get-and-set::delete_single_state: Error calling skylark manage delete state: {:?}",
                        e
                    );
                    Ok(Response::builder()
                        .status(StatusCode::INTERNAL_SERVER_ERROR)
